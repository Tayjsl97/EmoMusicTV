...
def train(melody,chord,valence,step,allStep):
    coefficient = (1 / allStep) * step
    P_coefficient=(1 / allStep) * step*0.05
    ...
    KL_loss,b_KL_loss,root,type=VAE(melody,chord,s_p,S_B)
    GT_chords=chord.narrow(1,1,chord.shape[1]-1)
    _, GT_type = GT_chords.narrow(-1, 0, 7).topk(1)
    _, GT_root = GT_chords.narrow(-1, 7, 41).topk(1)
    GT_type=GT_type.squeeze(-1)
    GT_root=GT_root.squeeze(-1)
    l = 0
    for k in range(12):
        l += criterion(type[k], GT_type[k])
        l += criterion(root[k], GT_root[k])
    ...
    for epoch in range(0,Epoch):
        ...
        while i < train_length:
            ...
            for j in range(batch_size // 12):
                ...
                rl_total+=rl*12;kl_total+=kl*12;P_kl_total+=P_kl*12
                temp_rl_loss+=rl*12;temp_kl+=kl*12;temp_Pkl+=P_kl*12
                train_start_idx+=12
            i = i + j + 1
            if loss_update!=0:
                optimizer.zero_grad()
            ...
            if train_start_idx%(print_every)==0:
                print('epoch train:%d, %s(%d %d%%) total: %.6f rl: %.6f Pkl: %.6f kl: %.6f' % (
                    epoch, timeSince(start_time), train_start_idx, train_start_idx / (train_length  * 12) * 100,
        ...
        if test_average < max_test_loss:
            ...
            model_save_path = model_path + "chordVAE_" + dataset + "_epoch" + str(epoch) + "_min_" + str(round(test_average,4)) + ".pth"
            state = {'model': VAE.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
            torch.save(state, model_save_path)
            max_test_loss = test_average
        else:
            if epoch%5==0:
                model_save_path = model_path + "chordVAE_" + dataset + "_epoch" + str(epoch) + "_" + str(round(test_average,4)) + ".pth"
                state = {'model': VAE.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                torch.save(state, model_save_path)
        f.close()
...
if __name__ == '__main__':
    ...
    patience = 20
    Epoch = 200
    VAE = chordVAE(N=3,h=4,m_size=8,c_size=48,d_ff=256,hidden_size=256,latent_size=128,dropout=0.2).to(device)
